...
import { getOnPrimary, staticPrimaryColor, staticPrimaryDarkenColor } from '@/plugins/vuetify/theme'
...
const colors: { main: string; darken: string }[] = [
  ...
  { main: '#FFB400', darken: '#FFB400' },
  { main: '#0D9394', darken: '#0C8485' },
  { main: '#8C57FF', darken: '#7E4EE6' },
...
const setPrimaryColor = useDebounceFn((color: { main: string; darken: string }) => {
  ...
  // update readable foreground for primary immediately
  vuetifyTheme.themes.value[vuetifyTheme.name.value].colors['on-primary'] = getOnPrimary(color.main)
...
const resetCustomizer = async () => {
  if (isCookieHasAnyValue.value) {
    ...
    // ensure on-primary matches the reset primary
    vuetifyTheme.themes.value.light.colors['on-primary'] = getOnPrimary(staticPrimaryColor)
    vuetifyTheme.themes.value.dark.colors['on-primary'] = getOnPrimary(staticPrimaryColor)
...
.app-customizer {
  ...
  .app-customizer-primary-colors {
    display: flex;
    flex-wrap: wrap; /* allow color swatches to wrap to next row when there are more than fit */
    row-gap: 0.5rem; /* vertical spacing between wrapped rows */
