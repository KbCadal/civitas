...
import type { Data } from '@db/dstipro/datatable/types';
...
function savePost() {
  if (modalMode.value === 'add') {
    ...
    if (index > -1) items.value[index] = { ...selectedPost.value! };
  }
...
<template>
  <div v-if="isAuthenticated">
    <VRow>
      <VCol cols="12">
        <VCard title="News" class="news">
          <VCardText class="d-flex align-center flex-wrap gap-4">
            <VRow>
              <VCol cols="12" md="8" sm="12">
                <VBtn prepend-icon="ri-add-line" class="bg-primary" color="white" @click="openModal('add')">Tambah Data
                </VBtn>
              </VCol>

              <VCol cols="12" md="4" sm="12">
                <VTextField v-model="searchText" label="Search" placeholder="Search ..."
          ...
          <VDialog v-model="showModal" persistent :max-width="$vuetify.display.smAndDown ? 'auto' : 500">
            ...
            <VCard class="pa-sm-3 pa-3">
              ...
              <VCardText>
                <VRow>
                  <VCol cols="12" md="12">
                    <VTextField v-model="selectedPost.title" label="Judul" dense :readonly="modalMode === 'view'" />
                  </VCol>
                  <VCol cols="12" md="12">
                    <VTextarea v-model="selectedPost.body" label="Isi" dense :readonly="modalMode === 'view'" />
                  </VCol>
                  <VCol cols="12" md="12">
                    <VTextField v-model="selectedPostTags" label="Tag" dense :readonly="modalMode === 'view'" />
                  </VCol>
                </VRow>
              </VCardText>
              ...
              <VCardActions>
                ...
                <VBtn v-if="modalMode !== 'view'" class="bg-success" color="white" variant="tonal" @click="savePost">
                  Simpan
                </VBtn>
              </VCardActions>
            </VCard>
          ...
          <VDialog v-model="showDeleteModal" persistent :max-width="$vuetify.display.smAndDown ? 'auto' : 400">
            ...
            <VCard class="pa-sm-7 pa-3">
              ...
              <VCardActions class="ma-n3">
                ...
                <VBtn variant="tonal" class="bg-error" color="white" @click="deletePost">Hapus</VBtn>
              </VCardActions>
            </VCard>
          </VDialog>
        </VCard>
