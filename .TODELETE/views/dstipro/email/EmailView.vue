...
import type { MoveEmailToAction } from '@/views/dstipro/email/useEmail'
import { useEmail } from '@/views/dstipro/email/useEmail'
  ...
  <VNavigationDrawer temporary :model-value="!!props.email" location="right" :scrim="false" floating class="email-view">
    <template v-if="props.email">
      ...
      <div class="email-view-header d-flex align-center px-5 py-4">
        <IconBtn class="me-2 flip-in-rtl"
        ...
        <div class="d-flex align-center flex-wrap flex-grow-1 overflow-hidden gap-2">
          ...
          <div class="d-flex flex-wrap gap-2">
            <VChip v-for="label in props.email.labels" :key="label" :color="resolveLabelColor(label)" size="small"
        ...
        <div class="d-flex align-center gap-2">
          <IconBtn variant="plain" :disabled="!props.emailMeta.hasPreviousEmail" class="flip-in-rtl"
            @click="$emit('navigated', 'previous')">
            <VIcon icon="ri-arrow-left-s-line" />
          </IconBtn>
          <IconBtn variant="plain" class="flip-in-rtl" :disabled="!props.emailMeta.hasNextEmail"
      ...
      <div class="email-view-action-bar d-flex align-center text-medium-emphasis gap-1 px-5">
        ...
        <div class="d-flex align-center gap-x-1">
          ...
          <IconBtn :color="props.email.isStarred ? 'warning' : 'default'"
            ...
            <VIcon :color="props.email.isStarred ? 'warning' : ''" icon="ri-star-line" />
          </IconBtn>
          <MoreBtn />
        </div>
      </div>
      ...
      <PerfectScrollbar tag="div" class="mail-content-container flex-grow-1 pa-sm-12 pa-6"
        ...
        <VCard v-if="showReplyBox">
          <VCardText>
            ...
            <TiptapEditor v-model="emailReply" placeholder="Write your message..." :is-divider="false" />
            <div class="d-flex justify-end gap-4 pt-2 flex-wrap">
              <IconBtn icon="ri-delete-bin-7-line"
                @click="showReplyBox = !showReplyBox; showReplyCard = !showReplyCard; emailReply = ''" />
              <VBtn variant="text" color="secondary">
                <template #prepend>
                  <VIcon icon="ri-attachment-2" class="text-high-emphasis" />
                </template>
                Attachments
              </VBtn>
